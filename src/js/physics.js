...
export class Physics {
  ...
  broadPhase(player, world) {
    ...
    // If they aren't empty, they are possible collision candidates.
  ...
  narrowPhase(candidates, player) {
    ...
    for (const block of candidates) {
      ...
      // Determine if the point is inside the player's bounding cylinder.
      ...
      if (this.pointInPlayerBoundingCylinder(closestPoint, player)) {
        ...
        // Compute the collision normal (pointing away from the contact point)
  ...
  pointInPlayerBoundingCylinder(p, player) {
    ...
    // Check if the contact point is inside the player's bounding cylinder.
    return (
  ...
  resolveCollisions(collisions, player) {
    ...
    for (const collision of collisions) {
      // We need to recheck if the contact point is still inside the player's bounding cylinder
      ...
      // Adjust the player's position so the block and player no longer overlap.
      let deltaPosition = collision.normal.clone();
      deltaPosition.multiplyScalar(collision.overlap);
      player.position.add(deltaPosition);

      // Negate the player's velocity along the collision normal.
