...
export class WorldChunk extends THREE.Group {
  ...
  /**
   * Determines if a resource should be generated at the given coordinates
   * @param {SimplexNoise} simplex
   * @param {number} x
   * @param {number} y
   * @param {number} z
  ...
  /**
   * Creates a tree depending on the biome at x, y, z
   * @param {RNG} rng
   * @param {string} biome
   * @param {number} x
   * @param {number} y
  ...
  /**
   * Generates a plane to simulate water
   */
  generateWater() {
    const material = new THREE.MeshLambertMaterial({
      color: 0x276cf5,
  ...
  generateMeshes() {
    ...
    const processBlocks = () => {
      ...
      while (
        ...
        blockIndex - startIndex < blocksPerFrame
      ) {
  ...
  removeBlock(x, y, z) {
    ...
    for (const [ax, ay, az] of adjacentBlocks) {
      ...
      if (
        a &&
        a.id !== blocks.empty.id &&
        a.instanceId === null &&
        !this.isBlockObscured(ax, ay, az)
      ) {
